...
export async function POST(req: Request) {
  ...
  let images_message = [];
  let prompt = `You are a creator who generates vivid, detailed stories based on images.`;
...
Using the specified story style and provided images, create a captivating, well-developed story with an engaging title and comprehensive descriptions of four storyboard frames for stable-diffusion-3.5-large to render as a storyboard sequence.`;
    ...
    prompt += `
${tone}
Using the specified story style and the keyword '${keyword}', create a captivating, well-developed story with an engaging title and comprehensive descriptions of four storyboard frames for stable-diffusion-3.5-large to render as a storyboard sequence.`;
  }
...
Tell a story that feels deeply personal and genuine, as if someone is sharing their experience directly with the listener. The tone should feel human and spontaneous, capturing emotions, thoughts, and small details that bring the story to life. Avoid a robotic, formulaic approach; instead, let the story unfold naturally, with a conversational flow and realistic pauses or reflections. This is about creating a vivid, relatable experience, not just recounting events—focus on immersing the listener in the storyteller's world.
  Please include the ${keyword} in the story.
...
- The title and content should be in ${language}, with frames described in English. Ensure the title is short yet captivating. The story content should be highly descriptive, and ranging between 300-700 characters to provide more narrative richness and context.
- You need to return the json format in the following format, the frames should be string and no space between them:
{
  ...
  const result = await streamObject({
    ...
    system: "You generate the story frame for an image.",
    messages: [
      {
        role: "user",
        content: [
